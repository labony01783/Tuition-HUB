  ...
  const { signIn,setUser } = useContext(AuthContext); 
   ...
   useEffect(() => {
    const checkAndNavigate = async () => {
        if (userRole === 'teacher') {
            const profileExists = await checkProfileExists(user.uid);  // Check profile existence
            if (profileExists) {
                navigate('/profile');
            } else {
                navigate('/AddProfile');
...
}, [userRole, user?.uid]);  // Ensure it runs when userRole or user ID changes
...
const handleLogin = async (e) => {
  ...
  try {
      ...
      const profileExists = await checkProfileExists(user.email);  // Check using Firebase user UID or email
